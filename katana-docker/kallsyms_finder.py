...
# This file is taken from the vmlinux-to-elf project
# https://github.com/marin-m/vmlinux-to-elf
# 
# It was slightly modified to write the kallsyms output format
# Katana needs for its analysis.
...
if __name__ == '__main__':
    ...
    fout = open("{}-kallsym".format(args.input_file) , "w")
    with open(args.input_file, 'rb') as kernel_bin:
        kallsyms = KallsymsFinder(kernel_bin.read(), 64)
        
        for symbol_address, symbol_name in zip(kallsyms.kernel_addresses, kallsyms.symbol_names):
            print(f'{symbol_name[1:]} {hex(symbol_address)}', file=fout)
